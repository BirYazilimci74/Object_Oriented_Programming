...
public class Game {
	...
	private static final int NEXT_PIECE_Y = 15;   // this changed

	public static final int STARTING_PIECE_X = 4;
	public static final int STARTING_PIECE_Y = 18; // this changed
	...
	private void tick() {
		...
		if (this.field.isPieceFloat(currentPiece)) {
			this.field.placePiece(currentPiece);
			this.field.destroyFullRows();
			if (!this.field.doesPieceTouchesBottom(currentPiece)){
	...
	public void movePieceUp() {
		if (!this.field.doesPieceTouchesCelling(this.currentPiece)) {
			this.currentPiece.moveUp();
			if (this.field.isPieceIntoBrick(this.currentPiece)) {
				this.currentPiece.moveDown();
			} else {
