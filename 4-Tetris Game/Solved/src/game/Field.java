...
public class Field {
	...
	private void initializeField(int fieldHeight, int fieldWidth) {
		...
		for (int row = 0; row < field.length; row++) {
			for (int col = 0; col < field[0].length; col++) {
				if (row > 8 && this.random.nextInt(11) <= -1) {       // -1, 5 ti. 5 i azalttıkça gereksiz blokların sıklığı azaldı.
					//this.field[row][col] = FieldSquare.STACK;             // bu if koşulu 8 den sonraki satırlarda rasgele bloklar oluşturuyor.
	...
	public boolean doesPieceTouchesCeiling(Piece piece)
	{                                                                     // I have written this method to check
		int pieceX = piece.getX();                                        //it touches to ceiling or not.
 		int pieceY = piece.getY();                                        //
		                                                                  // If it touches, returns true.
		PieceShape pieceShape = piece.getShape();                         // Otherwise returns false.
		int pieceHeight = pieceShape.getHeight();                         //
		int pieceWidth = pieceShape.getWidth();                           //
		                                                                  //
		for (int row = 0; row < pieceHeight; row++)
